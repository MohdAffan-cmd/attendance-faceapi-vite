...
app.get('/api/attendance', (req, res) => {
  if (!fs.existsSync(JSON_FILE)) return res.status(200).send([]);
  ...
  try {
    ...
    if (mode === 'update') {
      ...
      sheet.eachRow((row, rowNumber) => {
        ...
        if (row.getCell('name').value === name && row.getCell('date').value === date && !row.getCell('signOut').value) {
          targetRow = row;
        }
      });
    ...
    if (mode === 'update') {
      ...
      if (index !== -1) {
        ...
        if (signOutTimestamp) jsonData[index].signOutTimestamp = signOutTimestamp;
    ...
    return res.status(200).send({ message: 'Files updated' });
  } catch (error) {
    console.error('Update error:', error);
    return res.status(500).send({ error: 'Failed to update files' });
  }
});
